(() => {
    ...
    function updateSlides() {
        ...
        if (params.cssMode) {
            utils_setCSSProperty(wrapperEl, "--swiper-slides-offset-before", `${offsetBefore}px`);
            utils_setCSSProperty(wrapperEl, "--swiper-slides-offset-after", `${offsetAfter}px`);
        }
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".case__slider", {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function updateMobileLine() {
            ...
            const end = vh * .2;
            if (rect.top > start) {
                items.style.setProperty("--line-progress", "0px");
    ...
    document.querySelectorAll(".testimonials__sponsors").forEach(container => {
        const track = container.querySelector(".sponsors-testimonials__track");
        if (!track) return;
        let pos = 0;
        let speed = .6;
        let paused = false;
